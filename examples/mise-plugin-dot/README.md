...
A [mise](https://mise.jdx.dev) plugin that installs and manages the `dot` CLI from any chezmoi-managed dotfiles repository — including [YuriiF/dotfiles](https://github.com/YuriiF/dotfiles) — through a single one-liner.

## What it does

```sh
mise install dot@latest                          # the maintainer's reference repo
mise install dot@YuriiF/dotfiles      # explicit owner/repo
mise install dot@v0.2.502                        # pinned tag
mise install dot@alice/cfg                       # any user's dotfiles
mise use -g dot@v0.2.502                         # set globally
```

Under the hood the plugin invokes `dot init <owner/repo>` so every install runs through the framework's signed-bootstrap pipeline (see [HARD_AUDIT_2026 §H6](https://github.com/YuriiF/dotfiles/blob/main/docs/operations/HARD_AUDIT_2026.md)). No unverified `curl | sh`.
...
4. devtools.fm episode pitch sent to Jeff Dickey (per [ROADMAP_2026 §E1](https://github.com/YuriiF/dotfiles/blob/main/docs/operations/ROADMAP_2026.md)).
