...
- [`main.json`](main.json) — protection ruleset for the `main`
  ...
  under [#857](https://github.com/YuriiF/dotfiles/issues/857):
...
```bash
# First-time creation:
gh api -X POST repos/YuriiF/dotfiles/rulesets \
...
gh api -X PUT repos/YuriiF/dotfiles/rulesets/<id> \
  --input .github/rulesets/main.json
```
...
```bash
gh api repos/YuriiF/dotfiles/rulesets/<id> | jq '.rules[]'
```
...
- [#857](https://github.com/YuriiF/dotfiles/issues/857) — promote `Compliance Summary` + `Reliability Summary` to required checks.
- [#853](https://github.com/YuriiF/dotfiles/issues/853) — Rulesets-as-code for signed commits (this file already encodes it).
